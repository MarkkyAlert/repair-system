...
- **ผู้ใช้ 5 กลุ่ม:**
  ...
  - **หัวหน้างาน (manager)** — อนุมัติ มอบหมาย/ย้ายงาน ดูภาพรวมทั้งองค์กร
  - **ผู้ดูแลระบบ (admin)** — ตั้งค่าทั้งหมด จัดการผู้ใช้ ดูรายงาน ปิดงานแทนผู้แจ้งได้เมื่อจำเป็น
  - **ผู้เยี่ยมชม/แขก (guest)** — สแกน QR ที่เครื่อง แจ้งซ่อมได้โดยไม่ต้องมีบัญชี

## 2. ฟีเจอร์หลัก (จาก source จริง)

- **แจ้งซ่อม + ติดตามสถานะ** พร้อมแนบรูป, ประวัติงาน, การเปิดงานซ้ำ (reopen), ให้คะแนนความพึงพอใจ (CSAT)
- **SLA อัตโนมัติ** — คำนวณกำหนดเวลาตาม priority/หมวดหมู่; หน้าจออ่านกำหนดเวลาได้ทันที และ cron จะบันทึกผลเกินกำหนดพร้อมส่งการแจ้งเตือน/อีเมล
...
| ฐานข้อมูล | MySQL 8.0+ / MariaDB 10.3+ (ผ่าน PDO) |
...
- **PHP 8.2–8.5 แบบ 64-bit** (PHP 8.6 ยังใช้ไม่ได้กับ dependency ชุดที่ล็อกไว้) พร้อมส่วนขยาย: `pdo, pdo_mysql, mbstring, gd, zip, zlib, dom, xml, simplexml, xmlreader, xmlwriter, libxml, fileinfo, iconv, ctype, filter, hash, openssl, json`
  - ต้อง **64-bit** เพราะ zipstream-php (มากับ PhpSpreadsheet) ประกาศ platform เป็น `php-64bit` โฮสต์ 32-bit จะติดตั้งไม่ผ่าน
- **MySQL 8.0+ / MariaDB 10.3+** (charset `utf8mb4`) — รายงานใช้ window functions ซึ่ง MySQL 5.7 ยังไม่รองรับ
- **เว็บเซิร์ฟเวอร์** ที่เปิด `mod_rewrite` และอ่าน `.htaccess` (Apache — มาตรฐาน cPanel)
- ตั้ง **cron ได้** (สำหรับ SLA + อีเมลอัตโนมัติ)

> เปิดหน้า `https://your-site/check-requirements.php` จะตรวจ PHP, รุ่นฐานข้อมูล, schema และสิทธิ์เขียนไฟล์ให้อัตโนมัติก่อนติดตั้ง
...
| `php bin/run-maintenance-cron.php` | งานเบื้องหลัง: mark SLA ที่เกินกำหนด + ส่งการแจ้งเตือน/อีเมลในคิว (ตั้งเป็น cron) |
...
- **ข้อจำกัดที่ควรรู้:**
  - ต้องตั้ง **cron** เพื่อบันทึก SLA ที่เกินกำหนดและส่งการแจ้งเตือน/อีเมลตามเวลา; ถ้าไม่ตั้ง หน้าจอยังเทียบกำหนดเวลาได้ แต่ผู้เกี่ยวข้องจะไม่ได้รับการแจ้งเตือนอัตโนมัติ
  - ต้อง PHP 8.2–8.5 แบบ 64-bit และเปิดส่วนขยายตามข้อ 4
  - การสำรองอัตโนมัติเต็มรูปแบบต้องการโฮสต์ที่เปิด `proc_open`/`mysqldump`; ถ้าปิด ให้ใช้ปุ่ม "สำรอง & ดาวน์โหลด" ในหน้า admin แทน
- **License:** สัญญาอนุญาตเชิงพาณิชย์ — **ใช้ได้ในองค์กรเดียว ห้ามขายต่อ/แจกจ่ายซอร์ส** ดูรายละเอียดใน [LICENSE.md](LICENSE.md)

### ⚠️ ก่อนส่งมอบ/ขาย — เจ้าของต้องเติมข้อมูลจริงเอง

รายการ placeholder ด้านล่างตั้งใจเว้นไว้ให้เจ้าของกรอก (ระบบเติมข้อมูลธุรกิจแทนไม่ได้) — เคลียร์ให้ครบก่อนส่งมอบ:

- [ ] **[LICENSE.md](LICENSE.md)** — เติม `[ชื่อผู้ขาย / ชื่อบริษัท]`, `[ปี]`, `[อีเมล / ช่องทางติดต่อ]`
- [ ] **Support เพิ่มเติม (ถ้าจะขาย)** — [LICENSE.md](LICENSE.md) ฉบับปัจจุบันกำหนดว่าไม่รวม support/update เว้นแต่ตกลงแยก; ถ้าจะเสนอเพิ่ม ให้ระบุช่องทาง ระยะเวลา ราคา และแก้ข้อตกลงให้ตรงกัน

> หมายเหตุ: `THIRD-PARTY-LICENSES.md` (สัญญา LICENSE ข้อ 5 อ้างถึง) มีอยู่ในแพ็กเกจแล้ว ไม่ต้องสร้างใหม่
