...
/**
 * คำขอนี้เป็นการรีเฟรชเบื้องหลังที่หน้าเว็บยิงเอง (ตัวจับเวลา / กลับมาโฟกัสแท็บ / สลับรายการอัตโนมัติ) ไม่ใช่คนตั้งใจทำ?
 *
 * ใช้ตัดสินว่าจะต่ออายุ session ให้ไหม. ดูแค่ "ยิงไป endpoint ไหน" ไม่พอ เพราะ endpoint เดียวกันถูกเรียกได้ทั้งจาก
 * การกดของคน (กดกระดิ่งเพื่อดูแจ้งเตือน = ใช้งานจริง ต้องต่ออายุ) และจากตัวจับเวลา (ไม่ใช่การใช้งาน ต้องไม่ต่อ);
 * กลับกัน หน้า /tickets ที่เป็นหน้าปกติ ก็ถูกดึงซ้ำเบื้องหลังตอนมีงานใหม่เข้าคิวเหมือนกัน. ฝั่ง JS จึงติดธงนี้มา
 * เฉพาะคำขอที่ไม่ได้เกิดจากคน — คำขอที่คนกดเองจะไม่มีธง แล้วนับเป็น activity ตามปกติ.
 *
 * ธงมาจาก client ก็จริง แต่ไม่ได้ให้สิทธิ์อะไรเพิ่ม: ผลของมันคือ "ไม่ต่ออายุ session ของตัวเอง" อย่างเดียว
 * และเครื่องที่ถูกทิ้งไว้เฉย ๆ ก็ไม่มีคำขอแบบไม่มีธงเกิดขึ้นอยู่แล้ว. endpoint ที่มีไว้ poll ล้วน ๆ ยังบังคับ
 * touchActivity: false ไว้ที่ฝั่ง server อีกชั้น ไม่พึ่งธงนี้.
 *
 * @param array<string, mixed>|null $server ค่าเริ่มต้นคือ $_SERVER
 */
function request_is_background_refresh(?array $server = null): bool
{
    $server ??= $_SERVER;

    return (string) ($server['HTTP_X_BACKGROUND_REFRESH'] ?? '') === '1';
}
...
/**
 * escape อักขระพิเศษของ LIKE (\ % _) ในคำค้นของผู้ใช้ ก่อนห่อด้วย %...% เพื่อให้ MySQL ตีความเป็นตัวอักษรจริง ไม่ใช่
 * wildcard. ต้อง escape backslash ก่อน (ไม่งั้นจะไปหนี % _ ที่เพิ่งใส่). sql_mode ปกติใช้ \ เป็น escape ของ LIKE
 * โดยดีฟอลต์อยู่แล้ว จึงไม่ต้องมี ESCAPE clause. เดิมค่าค้นที่มี _ (เช่นรหัสทรัพย์สิน/serial อย่าง "SN_2024") จับเกิน
 * ไปโดนแถวอื่น ("SN12024", "SNX2024") เพราะ _ เป็น wildcard หนึ่งตัวอักษร.
 */
