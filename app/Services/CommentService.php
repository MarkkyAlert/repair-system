...
class CommentService
{
    ...
    public function createComment(int $ticketId, array $viewer, array $input, array $files = []): void
    {
        ...
        // ส่งซ้ำด้วย token เดิม (กดปุ่มรัว/กด back แล้วส่งใหม่) จะไม่สร้าง comment ซ้ำ — ถูกต้องแล้ว. แต่ถ้ารอบที่ส่งซ้ำ
        // "แนบไฟล์เพิ่มมาด้วย" ไฟล์เหล่านั้นจะไม่ถูกเก็บ (อยู่ใน if ($created)) ทั้งที่ผ่านการตรวจไฟล์มาแล้ว —
        // เดิมผู้ใช้ได้ข้อความว่าบันทึกสำเร็จ ทั้งที่ไฟล์หายไปเงียบ ๆ. ต้องบอกตรง ๆ ว่าอะไรเข้าอะไรไม่เข้า
        // (ตัว comment ยังอยู่ครบจากการส่งครั้งแรก ผู้ใช้แค่ต้องแนบไฟล์ใหม่อีกรอบ)
        if (!$created && $validatedFiles !== []) {
            throw new DomainException('ข้อความนี้ถูกบันทึกไปแล้วก่อนหน้านี้ ระบบจึงไม่ได้เพิ่มไฟล์แนบที่ส่งมารอบนี้ กรุณาแนบไฟล์ใหม่อีกครั้ง');
        }
    ...
    public function updateComment(int $ticketId, int $commentId, array $viewer, array $input): array
    {
        ...
        return [
            ...
            // ใช้ helper ตัวเดียวกับตอน render หน้า ป้ายจะได้ไม่เปลี่ยนภาษาหลังผู้ใช้กดแก้ inline
