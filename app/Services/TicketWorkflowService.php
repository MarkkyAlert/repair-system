...
class TicketWorkflowService
{
    ...
    public function cancelTicket(int $ticketId, array $viewer, array $input): void
    {
        ...
        // งานที่ช่างลงมือซ่อมและสรุปผลไปแล้ว ห้ามจบด้วย "ยกเลิก" ซึ่งแปลว่าไม่เคยเกิดขึ้น. เส้นทางที่ไปถึงตรงนี้ได้คือ
        // resolved → เปิดซ้ำตอนช่างเดิมถูกปิดบัญชี (งานเด้งกลับเป็น approved เพราะไม่มีช่าง) → ผู้แจ้งกดยกเลิก.
        // ผลคือรายงานความน่าเชื่อถือของทรัพย์สินทิ้งการเสียจริงของรอบนั้นไป (cancelled ถูกตัดออกจากฐาน) แต่ชั่วโมง
        // แรงงานยังถูกนับ = สองรายงานเล่าคนละเรื่อง. ไม่ได้ทำให้งานค้างตลอดกาล: หัวหน้ามอบหมายช่างคนใหม่ได้จาก
        // สถานะ approved แล้วเดินจนจบตามปกติ (หรือแอดมินปิดแทนได้เมื่อกลับไปถึง resolved)
        if ($this->reads->hasReportedWork($ticketId)) {
            throw new DomainException('รายการนี้มีการซ่อมและสรุปผลไปแล้ว จึงยกเลิกไม่ได้ — กรุณามอบหมายช่างให้ดำเนินการต่อจนจบ');
        }
    ...
    private function requireRequesterTicket(int $ticketId, array $viewer): array
    {
        ...
        // ด่านนี้ใช้ร่วมกัน 3 ทาง (ตรวจรับ / เปิดใหม่ / ยกเลิก) และตรวจอย่างเดียวคือ "ผู้กดเป็นผู้แจ้งของใบนี้ไหม"
        // ข้อความจึงต้องบอกกฎนั้น ไม่ใช่ชื่อการกระทำใดการกระทำหนึ่ง — เดิมเขียนว่า "ยืนยันผลการซ่อม" ทำให้แอดมิน
        // ที่กดยกเลิกได้ข้อความคนละเรื่องกับปุ่มที่กด และอ่านเหมือนเป็นปัญหาสิทธิ์ทั่วไป ทั้งที่กฎคือเจ้าของใบงาน
        if (!$this->policy->canRequesterManageClosure($ticket, $viewer)) {
            throw new DomainException('เฉพาะผู้แจ้งของรายการนี้เท่านั้นที่ตรวจรับ เปิดใหม่ หรือยกเลิกได้');
        }
