<?= ($appName ?? 'ระบบแจ้งซ่อม') . PHP_EOL ?>
<?= str_repeat('=', function_exists('mb_strlen') ? mb_strlen((string) ($appName ?? 'ระบบแจ้งซ่อม')) : strlen((string) ($appName ?? 'ระบบแจ้งซ่อม'))) . PHP_EOL . PHP_EOL ?>สวัสดี <?= (string) ($recipientName ?? 'ผู้ใช้งาน') . PHP_EOL . PHP_EOL ?><?= (string) ($heading ?? '-') . PHP_EOL ?><?= (string) ($intro ?? '-') . PHP_EOL . PHP_EOL ?><?= (string) ($message ?? '-') . PHP_EOL . PHP_EOL ?><?php if (!empty($sections) && is_array($sections)): ?>รายละเอียด<?php foreach ($sections as $section): ?>
