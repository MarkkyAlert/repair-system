<?php
// คู่มืออ่านรายงาน — เนื้อหา static ต่อยอดจาก description กับ field-hint ของแต่ละหน้ารายงาน
$groups = [
    'ภาพรวม & ผู้บริหาร' => [
        [
...
// เกณฑ์สี (tone) — ต้องตรงกับ threshold ในโค้ด มี tests/cases/report_guide_test.php คอยล็อกไว้
// ค่าที่ "ดี" ต่างกันตามเมตริก: SLA/completion/คะแนน ยิ่งสูงยิ่งดี ส่วน %เปิดซ้ำ/%เกิน SLA/คะแนนความเสี่ยง ยิ่งต่ำยิ่งดี
$toneRules = [
...
$caveats = [
    ...
    'งวดที่จบแล้วตรึงยอดปิด · MTTR · SLA · คะแนนตามข้อมูลที่เกิดไม่เกินวันสิ้นงวด งานที่เพิ่งปิด/เปิดซ้ำ/ให้คะแนนภายหลังจะไม่ย้อนแก้ภาพเดิม',
    'งานค้างใน Backlog/ภาระงานช่าง และชั่วโมงแรงงานสะสม เป็นภาพสถานะปัจจุบัน จึงเปลี่ยนได้แม้เลือก ticket จากช่วงอดีต — ไม่ใช่ตัวเลขงวดแบบตรึง',
    'Export: คอลัมน์ % ใน Excel เป็นตัวเลขจริง (pivot/sum ต่อได้) · PDF ฝังฟอนต์ไทยแล้ว (ส่งบอร์ดอ่านออก ไม่เป็นกล่อง)',
];

// อภิธานศัพท์ metric ที่ตีความผิดง่าย — นิยาม/สูตร + ทิศทางที่ดี + base ตัวเลขคำนวณถูก แต่อ่านผิดได้ถ้าไม่รู้นิยาม
$glossary = [
    ...
    ['term' => 'อัตราปิดงาน (completion) — เฉพาะสรุปผู้บริหาร', 'good' => 'ยิ่งสูงยิ่งดี',
        'def' => 'ปิด ÷ ticket ที่แจ้งทั้งหมดในงวด (นับตามวันแจ้ง) โดยดูสถานะ ณ วันสิ้นงวด — เมื่อพ้นวันสิ้นงวดแล้ว งานที่เพิ่งปิดหรือเปิดซ้ำภายหลังจะไม่ย้อนแก้อัตราเดิม. รายงานผลงานช่าง "ไม่มี" อัตราปิดงานรายคน เพราะตัวหาร (งานที่ช่างรับ) เปลี่ยนได้เมื่อโยกงาน/เปิดซ้ำ จึงประเมินคนย้อนหลังไม่ยุติธรรม — ดูผลงานช่างจาก "จำนวนปิดจริง + เวลาซ่อม + SLA + คะแนน" แทน'],
    ['term' => 'ผลงานช่าง — ตัวไหนนิ่ง ตัวไหนเป็นสถานะปัจจุบัน', 'good' => '—',
        'def' => 'คอลัมน์ประเมินผลงาน (ปิดงาน · เวลาซ่อม · SLA ตรงเวลา · คะแนน) คิดจาก "งานที่ช่างคนนั้นปิดจริงในช่วง" ให้เครดิตคนกดปิด + อ่าน SLA/คะแนนจากรอบที่ปิดจริง จึง **ไม่เปลี่ยนย้อนหลัง** แม้ภายหลังมีการเปิดซ้ำ/โยกงาน และหน้าสรุปรวมกับหน้าเต็มตรงกัน. ช่างที่ถูกปิดบัญชี/ปลดจากตำแหน่งไปแล้วก็ยังแสดงผลงานเดิม (ไม่ถูกลบทิ้ง). ส่วน "งานค้างปัจจุบัน · สัดส่วนโหลด · ค้างเก่าสุด" เป็น **สถานะ ณ ปัจจุบัน** (ใช้เกลี่ยงาน ไม่ใช่ประเมินย้อนหลัง). ระบบไม่แสดง "เวลาตอบรับ/ชั่วโมงแรงงาน/รับ" รายช่าง เพราะผูกกับสถานะปัจจุบัน/ช่วงวันที่ ทำให้นิ่งย้อนหลังไม่ได้'],
    ['term' => 'ยอดปิดงาน — นับต่างกันตามหน้า (โดยตั้งใจ)', 'good' => '—',
        'def' => 'สรุปผู้บริหาร = สถานะ ณ วันสิ้นงวดของ ticket ที่แจ้งในงวด (งวดอดีตไม่เปลี่ยนย้อนหลัง) · แนวโน้ม = นับตามงวดที่ปิดจริง 1 ครั้งต่อ 1 งาน · ผลงานช่าง = ให้เครดิต "คนที่กดปิด" (ถ้าปิด–เปิดซ้ำ–ปิดใหม่คนละคน นับให้ทั้งสองคน) — ตัวเลขจึงต่างกันได้ตามมุมมอง'],
];
?>
