...
$caveats = [
    ...
    'ช่วงเวลาสั้น (ไม่กี่วัน) ตัวเลขแกว่งง่ายเพราะจำนวนตัวอย่างน้อย — ดูหน้าแนวโน้มหลายงวดประกอบ',
    ...
    'งวดที่จบแล้วตรึงยอดปิด · เวลาซ่อมเฉลี่ย · SLA · คะแนนตามข้อมูลที่เกิดไม่เกินวันสิ้นงวด งานที่เพิ่งปิด/เปิดซ้ำ/ให้คะแนนภายหลังจะไม่ย้อนแก้ภาพเดิม',
    'งานค้างใน Backlog/ภาระงานช่าง และชั่วโมงแรงงานสะสม เป็นภาพสถานะปัจจุบัน จึงเปลี่ยนได้แม้เลือกงานแจ้งซ่อมจากช่วงอดีต — ไม่ใช่ตัวเลขงวดแบบตรึง',
    'การส่งออก: คอลัมน์ % ใน Excel เป็นตัวเลขจริง (pivot/sum ต่อได้) · PDF ฝังฟอนต์ไทยแล้ว (ส่งบอร์ดอ่านออก ไม่เป็นกล่อง)',
];

// อภิธานศัพท์ metric ที่ตีความผิดง่าย — นิยาม/สูตร + ทิศทางที่ดี + base ตัวเลขคำนวณถูก แต่อ่านผิดได้ถ้าไม่รู้นิยาม
$glossary = [
    ['term' => 'SLA (Service Level Agreement) — ข้อตกลงระดับการให้บริการ', 'good' => 'ยิ่งทำได้ตามกำหนดยิ่งดี',
        'def' => 'กำหนดเวลาที่ต้อง "ตอบรับ" งานและ "ซ่อมให้เสร็จ" นับจากเวลาแจ้ง ตั้งแยกตามระดับความสำคัญ (ผู้ดูแลระบบตั้งได้ที่หน้าความสำคัญ/หมวดหมู่) นับต่อเนื่องแบบ 24 ชม. ไม่หักเวลานอกเวลาทำการ. "เกิน SLA" = งานที่พ้นกำหนดแล้วยังไม่ถึงสถานะที่ต้องการ ไม่ได้แปลว่าใครทำผิด'],
    ...
    ['term' => '%เกิน SLA (พื้นที่ปัญหา)', 'good' => 'ยิ่งต่ำยิ่งดี',
        'def' => 'งานที่พลาดกำหนดในช่วง ÷ งานที่ตัดสินผลได้แล้ว (ทันกำหนด + พลาดกำหนด) — งานที่ยังไม่ถึงกำหนดไม่ถูกนับทั้งสองฝั่ง และถ้ายังตัดสินอะไรไม่ได้เลยจะเป็น "-" ไม่ใช่ 0%. นับที่ระดับ "ใบงาน" (ใบไหนมี SLA รอบใดพลาดถือว่าใบนั้นเกิน) เพื่อชี้ว่าพื้นที่/มิติไหนมีงานสะดุด SLA บ่อย — ต่างจากหน้า "วิเคราะห์ SLA เกินกำหนด" ที่นับระดับ "รายการ SLA" (แยกตอบรับ/แก้ไข) เปอร์เซ็นต์สองหน้าจึงไม่จำเป็นต้องเท่ากัน เป็นคนละมุมโดยตั้งใจ. ตัวเลขในวงเล็บข้างเปอร์เซ็นต์ = จำนวนงานที่เป็นฐาน'],
    ['term' => 'ยอดปิดงาน — นับต่างกันตามหน้า (โดยตั้งใจ)', 'good' => '—',
        'def' => 'สรุปผู้บริหาร = สถานะ ณ วันสิ้นงวดของงานแจ้งซ่อมที่แจ้งในงวด (งวดอดีตไม่เปลี่ยนย้อนหลัง) · แนวโน้ม = นับตามงวดที่ปิดจริง 1 ครั้งต่อ 1 งาน · ผลงานช่าง = ให้เครดิต "คนที่กดปิด" (ถ้าปิด–เปิดซ้ำ–ปิดใหม่คนละคน นับให้ทั้งสองคน) — ตัวเลขจึงต่างกันได้ตามมุมมอง'],
];
?>
