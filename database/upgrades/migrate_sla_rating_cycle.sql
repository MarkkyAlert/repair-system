-- ============================================================
-- รายงานตามที่เกิดจริง — เก็บ SLA กับคะแนนแยกตามรอบ (per-cycle snapshot)
-- ------------------------------------------------------------
-- เพิ่มคอลัมน์ `cycle` ให้ ticket_sla_tracks + ticket_ratings เพื่อให้ตัวเลขรายงานของงวดที่ผ่านไปแล้วนิ่ง ไม่เปลี่ยนย้อนหลัง:
--   เดิม พอ reopen จะ reset แถว SLA และการให้คะแนนใหม่ก็เขียนทับคะแนนเดิม → SLA/ความพึงพอใจ
--   ของเดือนที่ปิดไปแล้วเลย "เปลี่ยนย้อนหลัง" ตอนนี้ reopen/re-rate จะเพิ่มรอบใหม่ (cycle N+1) แทนการทับของเดิม
--   รายงานเลยอ่านค่าของรอบที่ปิดจริงในแต่ละงวด ส่วนภาพที่แคปเดือนเก่าไว้ก็อ่านค่าเดิมตลอด
--
...
--
-- ⚠️ Backfill รอบล่าสุด: งานที่ "ปิด–เปิดซ้ำ–ปิดใหม่" มาก่อนอัปเกรด จะเหลือ snapshot SLA/rating
