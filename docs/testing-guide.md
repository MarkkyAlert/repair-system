...
ระบบมี 4 บทบาทหลักที่กำหนดให้บัญชีได้ (คอลัมน์ `users.role`) — นอกจากนี้มี `guest` ที่แจ้งผ่าน QR โดยไม่มีบัญชี (ไม่เก็บใน `users.role`) รวมเป็น 5 กลุ่มผู้ใช้
...
6. **ทดสอบสแกน QR**:
   ...
   - **ถ้ายังไม่ login** (ทรัพย์สินที่รับแจ้งได้): เห็นหน้าข้อมูล asset แบบ guest พร้อมปุ่มหลัก **"แจ้งปัญหา (ไม่ต้อง login)"** → ไปฟอร์มแจ้งแบบ guest ที่ `/scan/{token}/report` (มีปุ่มรอง "หรือเข้าสู่ระบบ (สำหรับช่าง/ผู้ดูแลระบบ)")
   - **ถ้า login แล้ว**: เห็นปุ่ม **แจ้งปัญหาจากทรัพย์สินนี้** → กดแล้วจะไป `/tickets/create?asset_id=...` พร้อม pre-fill ฟิลด์
   - **ถ้าทรัพย์สินถูกเลิกใช้/จำหน่ายแล้ว** (`retired`/`disposed`): ยังเปิดดูข้อมูลได้ แต่ระบบขึ้นข้อความ "ไม่เปิดรับแจ้งซ่อม" แทนปุ่มแจ้ง (ไม่ใช่ 404)
...
> ```
> */5 * * * * /ABSOLUTE/PATH/TO/php /ABSOLUTE/PATH/TO/bin/run-maintenance-cron.php
> ```
> - **บน cPanel:** เข้าเมนู *Cron Jobs* → Common Settings เลือก "Once Per Five Minutes" → วางคำสั่งข้างบน
> - **ต้องเป็น path เต็มทั้งสองส่วน** — cron ใช้ PATH แคบ ๆ ของมันเอง คำสั่งที่ขึ้นต้นด้วย `php` เฉย ๆ มักตายด้วย `php: command not found` แบบเงียบ ๆ (PHP CLI มักอยู่ที่ `/usr/local/bin/php` หรือ `/usr/bin/php` — ถามผู้ให้บริการโฮสต์)
> - **หา path ที่ถูกต้องของเครื่องคุณอัตโนมัติ:** เปิด `https://your-site/check-requirements.php` — หน้านั้นพิมพ์คำสั่ง cron ที่เติม path จริงไว้แล้ว
> - (ทางเลือก) สำรองฐานข้อมูลรายวัน: `0 2 * * * /ABSOLUTE/PATH/TO/php /ABSOLUTE/PATH/TO/bin/backup-database.php`
